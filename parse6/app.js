...
class MultiStepForm {
    ...
    const tabStepBar = document.createElement('div')

    this.steps.forEach(( step, index ) => {

        const tabStep = document.createElement('button')
        tabStep.classList.add("btn-tab")
        tabStep.dataset.step = index
        
        tabStepBar.appendChild( tabStep )

    })

    this.form.prepend( tabStepBar )
    ...
    this.form.querySelectorAll( '.act-btn' ).forEach( btn => {
    ...
    const currentStepElement = document.getElementById(this.steps[index]);
    currentStepElement.classList.toggle("active");
    ...
    this.form.querySelectorAll( '.btn-tab' ).forEach( ( btn, index ) => {
        btn.addEventListener( 'click', ( e ) => {
        e.preventDefault()
        this.showStep( index )
        })
    ...
    this.form.querySelectorAll( '.btn-tab' ).forEach( (btn, index) => {
        ...
        btn.disabled = this.currentStep === index ? true : false
        btn.textContent =  this.steps[ index ]

    })
    
    this.form.querySelectorAll( '.act-nav' ).forEach( btn => {
        
        const prev = btn.dataset.action == "prev" ? btn : false
        const next = btn.dataset.action == "next" ? btn : false

        if (prev)
        prev.disabled = this.currentStep === 0 ? true : false

        if (next)
        next.disabled = this.currentStep === this.steps.length - 1 ? true : false
...
document.querySelectorAll('.multistep').forEach(form =>{
