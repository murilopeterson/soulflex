...
function render(data){
    ...
    const totalByGroup = newproducts.reduce((acc, item) => {
        const key = `${item.model}-${item.feature}-${item.size}`;
        acc[key] = (acc[key] || 0) + item.qty;
        return acc;
    }, {});
    html += `<div class="products-total">`
    Object.entries(totalByGroup).forEach(([key, qty]) => {
        const [model, feature, size] = key.split('-');
        html += `<p>${qty}x ${model} ${feature} ${size}</p>`;
    });
    html += `</div>`
...
function gentag(input){
    ...
    data.forEach((item) => {
       const order = item.row
            ...
            html += `<h1 class="order-pair"></h1>`

            html += `<div class="product"><h2>${order[0]}</h2>
                    <h3>${order[1]}</h3></div>
